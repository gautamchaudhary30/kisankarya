...
function App() {
  ...
  const openMessages = item => { setMessageItem(item || { name: 'KisanKarya Seller', price: 'Marketplace chat' }); setSelectedItem(null); window.scrollTo({ top: 0, behavior: 'smooth' }) }
  const openMyListings = () => { setLoginOpen(false); setPostOpen(false); setSelectedItem(null); setMessageItem(null) }
  const addListing = listing => setMyListings(prev => [listing, ...prev])

  if (messageItem) return <div className="app-shell"><Messages seller={messageItem.seller || 'Ramesh Patidar'} item={messageItem} onBack={() => { setMessageItem(null); setSelectedItem(messageItem) }} /></div>
  if (selectedItem) return <div className="app-shell"><ListingDetail item={selectedItem} onBack={closeDetail} onMessage={() => openMessages(selectedItem)} /></div>
  if (window.location.hash === '#my-listings') return <div className="app-shell"><MyListings listings={myListings} onBack={() => { window.location.hash = '#home' }} /></div>
  ...
  return <div className="app-shell">
    ...
    <BottomNav onPost={()=>setPostOpen(true)} onLogin={()=>setLoginOpen(true)} onMessages={()=>openMessages()} onMyListings={()=>{ window.location.hash = '#my-listings' }} />
    {postOpen && <PostListingModal onClose={()=>setPostOpen(false)} onSubmit={addListing} onViewListings={()=>{ setPostOpen(false); window.location.hash = '#my-listings' }} />}
    {loginOpen && <LoginPreview onClose={()=>setLoginOpen(false)} onMyListings={()=>{ setLoginOpen(false); window.location.hash = '#my-listings' }} />}
    {!postOpen && !loginOpen && <button className="my-listings-floating" onClick={()=>{ window.location.hash = '#my-listings' }}>My Listings</button>}
    {myListings.length > 0 && <div className="my-listings-preview"><div><span>MY LISTINGS</span><b>{myListings.length} item{myListings.length > 1 ? 's' : ''} · {myListings[0].status}</b></div><button onClick={()=>{ window.location.hash = '#my-listings' }}>View →</button></div>}
  </div>
}
