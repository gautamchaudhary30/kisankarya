...
function App() {
  ...
  const chooseRole = (selectedRole) => setRole(selectedRole)
  ...
  return (
    <div className="site">
      <header className="header">
        <div className="container nav">
          ...
          <nav className={menu ? 'nav-links open' : 'nav-links'}>
            {['New Tractors', 'Used Tractors', 'Brands', 'Compare', 'Implements', 'News'].map((item) => <a href={`#${item.toLowerCase().split(' ')[0]}`} key={item}>{item} <ChevronDown size={14} /></a>)}
          </nav>
          <button className="login-btn" onClick={() => setLoginOpen(true)}><UserRound size={16}/> Login</button>
      ...
      <main id="top">
        <section className="hero">
          ...
          <div className="container hero-content">
            <div className="hero-copy">
              ...
              <div className="hero-search"><Search size={21}/><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search tractor, brand or HP..."/><button>Search</button></div>
        ...
        <section className="trust"><div className="container trust-grid"><div><ShieldCheck/><span><b>Verified Listings</b><small>Trusted tractor information</small></span></div><div><IndianRupee/><span><b>Best Price Discovery</b><small>Compare prices easily</small></span></div><div><GitCompare/><span><b>Easy Comparison</b><small>Compare specs side by side</small></span></div><div><Tractor/><span><b>All Major Brands</b><small>1000+ tractor models</small></span></div></div></section>
      ...
      {loginOpen && <div className="login-overlay" onClick={() => setLoginOpen(false)}><div className="login-modal" onClick={e => e.stopPropagation()}>
        <button className="close-login" onClick={() => setLoginOpen(false)}><X size={20}/></button>
        {!role ? <>
          <div className="login-icon"><UserRound/></div><p className="eyebrow">WELCOME TO KISANKARYA</p><h2>Login as</h2><p className="login-subtitle">Choose how you want to use KisanKarya.</p>
          <button className="role-card" onClick={() => chooseRole('user')}><span className="role-icon"><UserRound/></span><span><b>As a User</b><small>Find tractors, compare prices & explore listings</small></span><ArrowRight/></button>
          <button className="role-card" onClick={() => chooseRole('engineer')}><span className="role-icon"><Wrench/></span><span><b>As an Engineer</b><small>Add and manage tractor listings for customers</small></span><ArrowRight/></button>
        </> : <>
          <button className="back-role" onClick={() => setRole(null)}>← Change role</button><div className="login-icon">{role === 'engineer' ? <Wrench/> : <UserRound/>}</div><p className="eyebrow">{role === 'engineer' ? 'ENGINEER LOGIN' : 'USER LOGIN'}</p><h2>Continue as {role === 'engineer' ? 'Engineer' : 'User'}</h2><p className="login-subtitle">Enter your mobile number to continue.</p><label>Mobile Number</label><input className="mobile-input" type="tel" placeholder="10-digit mobile number" maxLength="10"/><button className="continue-btn">Send OTP <ArrowRight size={17}/></button>
        </>}
      </div></div>}
    </div>
  )
