...
const nearby = [
  { name: 'Battery Sprayer 16L', price: '₹2,500', distance: 2.4, seller: 'Ramesh Patidar', location: 'Rau, Indore', category: 'Sprayers', condition: 'Used - Good', description: '16 litre battery sprayer in working condition. Suitable for crop spraying and small farms.', image: 'https://images.unsplash.com/photo-1589923188900-85dae523342b?auto=format&fit=crop&w=900&q=85' },
  { name: 'Mini Power Weeder', price: '₹28,000', distance: 5.8, seller: 'Mohan Farm', location: 'Depalpur Road', category: 'Farm Tools', condition: 'Used - Good', description: 'Compact power weeder for inter-cultivation and small field operations.', image: 'https://images.unsplash.com/photo-1592982537447-7440770cbfc9?auto=format&fit=crop&w=900&q=85' },
  { name: 'Rotary Hand Cultivator', price: '₹1,800', distance: 8.1, seller: 'Suresh Yadav', location: 'Sanwer', category: 'Implements', condition: 'Used - Fair', description: 'Simple rotary cultivator for garden beds and light farm work.', image: 'https://images.unsplash.com/photo-1598512752271-33f913a5af13?auto=format&fit=crop&w=900&q=85' },
  { name: 'Diesel Water Pump', price: '₹4,800', distance: 18.7, seller: 'Babulal Farm', location: 'Mhow Road', category: 'Irrigation', condition: 'Used - Good', description: 'Diesel water pump suitable for farm irrigation and water transfer.', image: 'https://images.unsplash.com/photo-1586771107445-d3ca888129ff?auto=format&fit=crop&w=900&q=85' },
]

export default function Home({ onPost, onSelect }) {
  ...
  return <main id="home">
    <section className="hero-mobile"><div className="hero-mobile-photo"/><div className="hero-mobile-shade"/><div className="hero-mobile-content">
      ...
      <span className="hero-kicker">KISANKARYA MARKETPLACE</span><h1>Kheti ke har kaam ka<br/><strong>apna marketplace.</strong></h1>
    ...
    <section className="nearby-section"><div className="section-title-row"><div><span className="section-kicker"><Navigation size={13}/> NEAR YOU</span><h2>Aapke aas-paas ke items</h2><p>{visibleNearby.length} listings found within {radius} km of Indore</p></div><button className="filter-circle"><SlidersHorizontal size={17}/></button></div>
      ...
      <div className="nearby-list">{visibleNearby.map(item=><article className="nearby-card" key={item.name} onClick={()=>onSelect?.(item)}><div className="nearby-image"><img src={item.image} alt={item.name}/><span><MapPin size={11}/> {item.distance} km</span></div><div className="nearby-info"><div className="item-top"><h3>{item.name}</h3><button aria-label="Save listing" onClick={e=>e.stopPropagation()}>♡</button></div><strong>{item.price}</strong><p>{item.location} · {item.seller}</p><small>Posted today · <b>Nearby</b></small></div></article>)}</div>
