...
const nearby = [
  { name: 'Battery Sprayer 16L', price: '₹2,500', distance: '2.4 km', seller: 'Ramesh Patidar', location: 'Rau, Indore', image: 'https://images.unsplash.com/photo-1589923188900-85dae523342b?auto=format&fit=crop&w=900&q=85' },
  { name: 'Mini Power Weeder', price: '₹28,000', distance: '5.8 km', seller: 'Mohan Farm', location: 'Depalpur Road', image: 'https://images.unsplash.com/photo-1592982537447-7440770cbfc9?auto=format&fit=crop&w=900&q=85' },
  { name: 'Rotary Hand Cultivator', price: '₹1,800', distance: '8.1 km', seller: 'Suresh Yadav', location: 'Sanwer', image: 'https://images.unsplash.com/photo-1598512752271-33f913a5af13?auto=format&fit=crop&w=900&q=85' },
]
...
export default function Home({ onPost }) {
  return <main id="home">
    <section className="hero-mobile">
      ...
      <div className="hero-mobile-shade" />
    ...
    <section className="nearby-section">
      <div className="section-title-row">
        <div><span className="section-kicker"><Navigation size={13}/> NEAR YOU</span><h2>Aapke aas-paas ke items</h2><p>Selected radius ke andar available listings</p></div>
        <button className="filter-circle"><SlidersHorizontal size={17}/></button>
      </div>
      <div className="radius-row"><span className="radius-label">Radius</span>{['5 km','10 km','25 km','50 km'].map((r,i)=><button className={i===1?'radius active':'radius'} key={r}>{r}</button>)}</div>
      <div className="nearby-list">{nearby.map(item=><article className="nearby-card" key={item.name}><div className="nearby-image"><img src={item.image} alt={item.name}/><span><MapPin size={11}/> {item.distance}</span></div><div className="nearby-info"><div className="item-top"><h3>{item.name}</h3><button>♡</button></div><strong>{item.price}</strong><p>{item.location} · {item.seller}</p><small>Posted today · <b>Nearby</b></small></div></article>)}</div>
      <button className="outline-wide">View all nearby listings <ArrowRight size={15}/></button>
    </section>

    <section className="category-section">
      <div className="section-title-row"><div><span className="section-kicker">EXPLORE</span><h2>What are you looking for?</h2></div><button className="link-btn">See all</button></div>
      <div className="category-scroll">{categories.map(c=><button className="category-tile" key={c.label}><span>{c.icon}</span><b>{c.label}</b></button>)}</div>
    </section>

    <section className="quick-post-banner">
      <div><span className="section-kicker light">FARMER SELLING</span><h2>Ghar par pada farm tool<br/>ab paise mein badlo.</h2><p>Photo upload karo, price set karo aur nearby farmers tak pahucho.</p></div>
      <button onClick={onPost}><span>＋</span> Post item</button>
    </section>
